...
argparser.add_argument('-c', '--config', required=False, help='path to the configuration file')
argparser.add_argument('-t', '--tracking_frames', required=False, help='number of frames without detection')
argparser.add_argument('--calibration', required=False, help='path to calibration configs')
...
model = YOLO(calib_paths=args['calibration'])
device = 'cpu'
