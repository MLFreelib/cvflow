...
### Запуск алгоритма
  ...
  Также скачать веса можно, запустив команду в терминале:
    ...
    wget "drive.google.com/u/3/uc?id=1cuWZ3t8H7iV5oUKGzU8wy8ucKGp0d64L&export=download&confirm=yes"

  Перейдя в терминале перед этим в дерикторию.

  1) Открыть директорию со скриптом

    cd examples/stereo_tracking
    
  2) Пример команды для запуска:
  
    python run.py --videofile ../../tests/test_data/top_l.mov,../../tests/test_data/top_r.mov --weights {путь к скачанным весам} --config ../../conf.txt
...
### Запуск алгоритма
  ...
  Перейдя в терминале перед этим в дерикторию, в которую хотите скачать веса.
  ...
  2) Пример команды для запуска:
    ...
    python run.py --images ../../tests/test_data/flaw_wood.jpg --weights {путь к скачанным весам} --temppath <путь к скачанным шаблонам>
...
1.Скачайте веса и подготовьте источники данных для определения размеров. Можно использовать данные из директории test_data. Поместите [веса](https://drive.google.com/drive/folders/1v1IsdUyqpYgn7ebLXpsd7Esmqb45bNMW) в директорию test_data для запуска примера.  
  Скачать веса можно, запустив команды в терминале:
    ...
    wget "drive.google.com/u/3/uc?id=1q5zBj6bFnR7zNQ9Rt_h3GOw6_p0MwWRs&export=download&confirm=yes"

  Перейдя в терминале перед этим в дерикторию, в которую хотите скачать веса.

3. Войти в директорию
...
Пример команды запуска c видео для распознавания размеров пузырей флотации: 
    ...
    python run.py -c 0.8 --videofile ../../tests/test_data/flotation.mp4 --weights ../../tests/test_data/unet.pt
